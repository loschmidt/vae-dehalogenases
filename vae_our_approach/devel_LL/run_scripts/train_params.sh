...
if [ $# == 0 ]; then
  ...
  cd $PYTHONSCRIPTS
  #python3 msa_filter_scorer.py --Pfam_id PF00561 --ref P59336_S14 --output_dir dimensionalityTest --stats --in_file results/PF00561/MSA/identified_targets_msa.fa
  ...
  for c; do
    ...
    echo "Running qsub on GPU node with C value ${c}"
