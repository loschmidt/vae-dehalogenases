""" Originally from Learning Meaningful representation  """
...
class DiscreteCurve(BasicCurve):
    ...
    def __call__(self, t):
        ...
        idx = torch.floor(t.flatten() * num_edges).clamp(min=0,
                                                         max=num_edges - 1).long()  # use this if nodes are equi-distant
...
class CubicSpline(BasicCurve):
    ...
    def __ppeval__(self, t, coeffs):
        ...
        tpow = t.reshape((-1, 1)).pow(
            torch.arange(0.0, degree, dtype=t.dtype, device=self.device).reshape((1, -1)))  # |t|x(degree)
    ...
    def deriv(self, t):
        coeffs = self.get_coeffs()  # Bx(num_edges)x4xD
        B, num_edges, degree, D = coeffs.shape
        dcoeffs = coeffs[:, :, 1:, :] * torch.arange(1.0, degree, dtype=t.dtype).reshape(1, 1, -1, 1).expand(B,
                                                                                                             num_edges,
                                                                                                             -1,
                                                                                                             D)  # Bx(num_edges)x3xD
