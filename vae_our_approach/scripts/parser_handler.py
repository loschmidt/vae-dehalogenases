...
class CmdHandler(metaclass=Singleton):
    def __init__(self, json_file=None):
        ...
        self.tree_nwk = self.args.tree_nwk
        self.tree_msa = self.args.tree_msa
    ...
    def get_parser(self):
        ...
        if ('--source_txt' not in unknown and len(unknown) > 0) and \
                ...
                ('--run_multicriterial_random_mutagenesis' not in unknown and len(unknown) > 0) and \
                ('--run_evolution' not in unknown and len(unknown) > 0) and \
                ('--run_package_stats_entropy' not in unknown and len(unknown) > 0):
